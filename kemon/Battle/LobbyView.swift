...
struct LobbyButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .overlay(
                Capsule().stroke(Color.kemonBlue.opacity(isHovered ? 1.0 : 0.6), lineWidth: 1.5)
            )
            .shadow(color: Color.kemonBlue.opacity(isHovered ? 0.6 : 0.3), radius: isHovered ? 12 : 6)
...
struct LobbyView: View {
    ...
    private var card: some View {
        VStack(spacing: 22) {
            ...
            HStack(spacing: 16) {
                KemonGlassButton(title: "Exit to Home", systemImage: "arrow.right.square") {
                    showExitConfirm = true
                }
                KemonPrimaryButton(title: "Resume", systemImage: "play.fill") {
                    battle.dismissLobby()
                }
            }
            .padding(.top, 4)
        ...
        .shadow(color: Color.kemonBlue.opacity(0.35), radius: 24)
    }
    ...
    private func turnRow(_ slot: BattleController.TurnSlot) -> some View {
        ...
        return HStack(spacing: 14) {
            ...
            Text("\(slot.id + 1)")
                ...
                .background(
                    Circle().fill(isSinging ? Color.kemonBlue.opacity(0.9)
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                .fill(isSinging ? Color.kemonBlue.opacity(0.14) : Color.white.opacity(0.05))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 16)
                .stroke(isSinging ? Color.kemonBlue.opacity(0.7) : Color.clear, lineWidth: 1.5)
        )
    }

    @ViewBuilder
    private func statusBadge(for slot: BattleController.TurnSlot) -> some View {
        ...
        HStack(spacing: 10) {
            ...
            case .singing:
                Text("SINGING")
                    ...
                    .background(Capsule().stroke(Color.kemonBlue, lineWidth: 1))
            case .upcoming:
                Text("UP NEXT")
                    .font(.poppinsBold(size: 10))
    ...
    private var exitConfirmOverlay: some View {
        ZStack {
            ...
            VStack(spacing: 22) {
                ...
                HStack(spacing: 16) {
                    KemonGlassButton(title: "Keep Playing") {
