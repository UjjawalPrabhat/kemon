...
struct VoiceScoringMatrix {
    ...
    /// One combined verdict mentioning both voice and vibe.
    func summary(for song: Song, emotionScore: Int) -> String {
        let overall = overall(emotionScore: emotionScore)
        let vibe = song.genre.displayName.lowercased()

        let opener: String
        switch overall {
        case 85...:     opener = "You nailed \"\(song.title)\"!"
        case 65..<85:   opener = "Strong performance on \"\(song.title)\"."
        case 40..<65:   opener = "You found it in flashes on \"\(song.title)\"."
        default:        opener = "Keep at \"\(song.title)\" — it'll come."
        }

        var clause = ""
        if let voice = normalizedScore {
            let pitch = inTuneness ?? voice
            clause = " Pitch \(pitch)%, and your expression \(emotionScore >= 65 ? "matched" : "reached for") the \(vibe) mood."
        } else {
            clause = " We couldn't hear your voice this time — scored on expression alone."
        }
        return "\(opener)\(clause) Overall: \(overall)%"
    }
