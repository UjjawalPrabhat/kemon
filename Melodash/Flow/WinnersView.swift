    ...
    private var leftColumn: some View {
        VStack(alignment: .leading, spacing: 40) {
            Text("The Final Winner")
                ...
                .foregroundStyle(
                    LinearGradient(
                        colors: [.white, Color.melodashBlue],
            ...
            HStack(spacing: 20) {
                MelodashDarkButton(title: "BACK TO HOME") { battle.reset() }
                MelodashPrimaryButton(title: "START NEW ROUND") { battle.startNewRound() }
            }
        }
    }
...
/// A dark frosted rounded-rect button, sized to sit beside `MelodashPrimaryButton`.
struct MelodashDarkButton: View {
    let title: String
    let action: () -> Void
