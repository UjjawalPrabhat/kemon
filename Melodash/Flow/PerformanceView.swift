...
struct PerformanceView: View {
    ...
    private var isLoading: Bool {
        !engine.isPerforming && !engine.didFinish
    }
    ...
    private var loadingOverlay: some View {
        VStack(spacing: 28) {
            ...
            SongArtworkView(song: song, size: 180, cornerRadius: 20)
                .shadow(color: Color.melodashBlue.opacity(0.3), radius: 20)
        ...
        .melodashPage(showPlanet: false, showCockpit: false, ufoStyle: .none)
    ...
    private var karaokeStage: some View {
        ZStack {
            ...
            Color.clear
                .melodashPage(showPlanet: true, showMoon: true, showCockpit: false, ufoStyle: .purpleRed)
        ...
        HStack(spacing: 14) {
            ...
            VStack(spacing: 7) {
                Text(playerName.isEmpty ? "Singer" : playerName)
                    ...
                    .overlay(Capsule().stroke(Color.melodashInkBlue, lineWidth: 1.5))
            ...
            .overlay(RoundedRectangle(cornerRadius: 18).stroke(Color.melodashCyan, lineWidth: 2))
            .shadow(color: Color.melodashCyan.opacity(0.5), radius: 10)
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(song.artist)
                        ...
                        .foregroundStyle(Color.melodashCyan)
                        .lineLimit(1)
                }
                Spacer(minLength: 0)
            }
            ...
            CameraPreview(session: engine.camera.session)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 14)
                        .stroke(Color.melodashBlue.opacity(0.4), lineWidth: 1.5)
                )
                ...
                .shadow(color: Color.melodashBlue.opacity(0.2), radius: 8)
    ...
    /// Toggles romanized lyrics. Lives in the bottom player bar (before the
    /// volume control) and only appears when the lyrics are non-Latin.
    private var romanizeButton: some View {
        Button {
            ...
            Image(systemName: "character.bubble")
                ...
                .foregroundStyle(showRomanized ? Color.melodashBlue : .white)
                .frame(width: 40, height: 40)
                .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
        .accessibilityLabel(showRomanized ? "Show original lyrics" : "Romanize lyrics")
        .help(showRomanized ? "Show original lyrics" : "Romanize lyrics")
    }
    ...
    private func avatarCircle(size: CGFloat) -> some View {
        avatarSquare(size: size * 0.8)
            ...
            .clipShape(Circle())
            .overlay(Circle().stroke(Color.melodashBlue.opacity(0.7), lineWidth: 1.5))
    }
    ...
    private var playbackBar: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 28)
                .stroke(Color.melodashCyan, lineWidth: 2)
        )
        .shadow(color: Color.melodashCyan.opacity(0.45), radius: 14)
    }
    ...
    private var volumesToggleButton: some View {
        Button {
            ...
            Image(systemName: "slider.horizontal.3")
                .font(.system(size: 22, weight: .medium))
                .foregroundStyle(showVolumePanel ? Color.melodashBlue : .white)
                .frame(width: 40, height: 40)
                .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
    ...
    /// Optional early-exit. The turn ends on its own when the track finishes
    /// (see `MelodashEngine.tick()` → `playback.didFinish`); this only lets a singer
