...
nonisolated final class CameraController: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, @unchecked Sendable {
    ...
    private let sessionQueue = DispatchQueue(label: "melodash.camera.session")
    private let frameQueue = DispatchQueue(label: "melodash.camera.frames", qos: .userInitiated)
