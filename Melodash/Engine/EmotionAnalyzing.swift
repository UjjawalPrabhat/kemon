...
//  whether or not the trained model is present. MelodashEngine loads
//  CoreMLEmotionAnalyzer if `MelodashEmotionClassifier.mlmodelc` is in the bundle,
...
/// below, name the exported model `MelodashEmotionClassifier.mlmodel`, and drop it
/// into the `Melodash/` folder. MelodashEngine picks it up automatically.
nonisolated final class CoreMLEmotionAnalyzer: EmotionAnalyzing, @unchecked Sendable {

    private let model: VNCoreMLModel

    /// Maps the classifier's raw class names (case-insensitive) onto Melodash's
    ...
    init() throws {
        // Xcode compiles MelodashEmotionClassifier.mlmodel → MelodashEmotionClassifier.mlmodelc.
        guard let url = Bundle.main.url(forResource: "MelodashEmotionClassifier",
